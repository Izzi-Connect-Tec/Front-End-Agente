...
export function acceptCall() {
  ...
  const activeConnections =
    contact?.getConnections().filter((conn) => conn.isActive()) || [];
  ...
  contact.accept({
    success: function () {
      console.log("Call accepted by button");
    },
    failure: function (err) {
      console.log("Error trying to accept call by button");
    },
  });
}
...
export function muteAgent() {
  ...
  if (contact.isMultiPartyConferenceEnabled() && activeConnections.length > 2) {
    ...
    if (connectingConnections.length === 0) {
      ...
      console.log(
        "Agent Connection cannot be muted while multi party participant is connecting"
      );
    }
  } else {
