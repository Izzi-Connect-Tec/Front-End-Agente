...
export default function TemporaryDrawer() {
  ...
  const notify = useCallback(() => {
    if (notificationsLoaded && actualizacion) {
      toast(`${notifications.at(-1).Titulo} 
       ${notifications.at(-1).Descripcion}`, {containerId: 'notifications'});
       setActualizacion(false);
    } else {
      console.log("Cambiando a cargasdos");
      setNotificationsLoaded(true)
    }
  }, [notifications, actualizacion]);
...
<ToastContainer containerId="notifications" stacked hideProgressBar={true}/>
